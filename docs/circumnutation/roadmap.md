...
| 14 | `add-circumnutation-pipeline` | **↑ Promoted ahead of PR #11 (2026-06-11)** — done next while Tier 4 is blocked on #230/Phase 2 (see the PR #11 row). `CircumnutationPipeline` DAG composition; pickle-ability; integration test on plate 001; CSV writer with full row-identity (CC-4). Should dedup the Tier 0/Tier 1 recompute that PR #10 `traveling_wave.compute` does redundant-by-design. | ⬜ | ⬜ |
