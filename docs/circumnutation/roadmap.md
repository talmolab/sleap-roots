...
| 9 | `add-circumnutation-tier3b-spatial-cwt` | Spatial CWT machinery (`cgau2` default): `resample_curvature` (non-uniform→uniform κ(s), the PR #8 handoff), `compute_scaleogram`, `extract_ridge` (λ_spatial ridge). Frozen Results, determinism canary, **no mask, no trait emission**. **`L_gz`/`L_c` DESCOPED** — the §7.4 |κ|-envelope-peak premise does not transfer to top-view tip-trail κ(s) (steady traveling wave; §6.5 inversion limit; empirically confirmed on all 6 plate-001 tracks) → follow-up research [#230](https://github.com/talmolab/sleap-roots/issues/230). 3 new constants (`SPATIAL_COI_EFOLDING_FACTOR`, `CWT_WAVELENGTH_{MIN_NYQUIST,MAX_SIGNAL}_*`); `_CONSTANTS_VERSION` 5→6. [#229](https://github.com/talmolab/sleap-roots/issues/229) | ⬜ | ⬜ |
| 10 | `add-circumnutation-tier3c-traits` | Tier-3 trait emission (theory.md §7.4) including `B_balance_number`, `L_gz_steady_state_residual`, `L_gz_resolvable`, `traveling_wave_residual`, `apex_basal_period_consistency`. **Applies the L_gz growth-zone mask** (CC-1). **⚠ Ripple from PR #9:** the `L_gz`-dependent traits (`B_balance_number`, `L_gz_steady_state_residual`, `L_gz_resolvable`) + the growth-zone mask are **blocked on [#230](https://github.com/talmolab/sleap-roots/issues/230)** (L_gz/L_c not yet measurable from tip-trail κ(s)); `traveling_wave_residual` + `apex_basal_period_consistency` remain deliverable from PR #9's λ(s_a) ridge (apply the cgau2 calibration map + the §7.4 unit handoff). | ⬜ | ⬜ |
