...
| 16 | `add-circumnutation-plots` | Scaleograms (Tier 1 + Tier 3); trail overlay (κ-color-coded); 6-up plate panel; `--no-plots` flag. `plotting` graduated **stub → impl** (impl 11 → 12, stub 2 → 1 — only `parametric`/PR #11 remains). `scaleogram` (polymorphic temporal/spatial, `LogNorm` power heatmap + COI dimming + ridge overlay), `trail_overlay` (signed-κ diverging `LineCollection`, image-y-down), `plate_panel` (2×3 shared-norm + one colorbar), `save_plots` orchestrator (re-derives via the exact tier helpers; writes `plots/` PNGs keyed on integer `track_id` + a `plots_metadata.json` provenance sidecar; `enabled` is the `--no-plots` hook for PR #17). Agg + structural/smoke tests (no pixel baselines); pure-pixel (CC-3). **Deviation:** `L_gz` arc-length marker omitted (blocked on #230). No new constants (`_CONSTANTS_VERSION` stays 6). Follow-up [#241](https://github.com/talmolab/sleap-roots/issues/241) (MCP-serializable Result artifacts). [#242](https://github.com/talmolab/sleap-roots/issues/242) · [PR #243](https://github.com/talmolab/sleap-roots/pull/243) | ✅ | ✅ |
| 17 | `add-circumnutation-cli` | `sleap-roots circumnutation analyze` — the user-facing CLI composing the whole pipeline on one `.slp` (load → adapt → `compute_traits` → write per-plant CSV → `aggregate_by_genotype` → write per-genotype CSV → `save_plots`), plus the `Series → CircumnutationInputs` adapter (formerly test-only). Two NEW modules `adapters` + `cli` (impl 12 → 14 by ADDITION; stub stays 1 — only `parametric`/PR #11). `adapters.series_to_inputs(series, *, …) -> (CircumnutationInputs, identity_provenance)`: prefix-anchored ASCII `[0-9]+` `track_id` coercion (raises rather than silently mis-coerce); CSV-as-source / flag-as-override metadata precedence (blank → absent); identity-provenance dict (`metadata_csv_path` + `metadata_csv_sha256` + a total per-field `identity_source` map `flag`/`metadata_csv`/`default`/`absent`); unmatched-`sample_uid` WARNING. `cli`: `circumnutation` click group + `analyze` (registered via `main.add_command`); `--sample-uid` required; gated per-genotype aggregation (`--no-aggregate`; genotype hard-error otherwise, before any output); `--no-plots` gates headless `matplotlib.use("Agg", force=True)`; `ClickException` error contract (exit 1 domain / 2 usage); `-v` logging to stderr (+ quiets matplotlib `findfont` noise); CC-3 (no `--px-per-mm`; `--help` → `convert_to_mm`). Output tree: top-level `run_metadata.json` + `per_plant/` + `per_genotype/` + `plots/`, all three sidecars byte-identical (single shared provenance snapshot); **sidesteps #238** via distinct subdirs (not closed). `_io.gather_run_metadata` gains `metadata_csv_path`/`metadata_csv_sha256`/`identity_source` (optional, backward-compatible). Synthetic-`.slp` round-trip + real plate-001 e2e; no new constants (`_CONSTANTS_VERSION` stays 6). Follow-up [#245](https://github.com/talmolab/sleap-roots/issues/245) (`save_plots` `input_path` provenance). [#244](https://github.com/talmolab/sleap-roots/issues/244) · [PR #246](https://github.com/talmolab/sleap-roots/pull/246) | ✅ | ✅ |
