...
| 15 | `add-circumnutation-per-genotype-aggregation` | `aggregate_by_genotype(per_plant_df, units) -> (per_genotype_df, units)` per theory.md §7.7 (median ± IQR; `n_plants_passing_qc`; exclusion reasons). NEW `aggregation` module (impl 10 → 11 by ADDITION). Groups by `(plate_id, genotype, treatment)`, plates not pooled (`dropna=False`); **73-column** output. `principal_axis_angle` dropped (wrapping circular angle); `helix_signed_area_px2` aggregated by magnitude; `growth_axis_unreliable` dropped. Fails loud on a track↔plant 1:1 violation (two-level per-plant collapse deferred to a follow-up). New `_io.write_per_genotype_csv`/`read_per_genotype_csv` + shared `_validate_units_coverage`. No new constants (`_CONSTANTS_VERSION` stays 6). | 🟡 | ⬜ |
