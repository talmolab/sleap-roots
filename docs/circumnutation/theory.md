...
Sign convention: positive = left turn, negative = right turn — this is the standard **y-up math** convention the formula above encodes (PR #8: in the **y-down image frame** the pipeline runs in, $+\kappa$ is a clockwise / visual-right turn *as displayed*; the implementation anchors on the **formula sign**, not the ambiguous word "left turn" — see Appendix B). **Cross-helper note (PR #8):** because the $\psi_g$ family (`_geometry.compute_psi_g` / `compute_signed_area` / the `handedness` trait) uses the swapped `atan2(dx, dy)`, the exact per-frame identity is $d\psi_g/dt = -\kappa\,|\mathbf{v}|$, so $\mathrm{sign}(\kappa) = -\,\texttt{handedness}$ for a single-signed-$\kappa$ loop — a PR #9/#10 chirality trait composing $\kappa$ with `handedness` must account for this opposite polarity. The denominator $|\mathbf{v}|^3$ diverges when the tip momentarily stops; at 5-min cadence on rice this can occur on individual frames where displacement is below noise. The pipeline must guard with $|\mathbf{v}| > k\sigma_\mathbf{v}$ for some $k$ (recommended $k=2$, masking sub-noise frames).
...
|---|---|
| $L_{gz}$ via peak-$|\kappa(s)|$ location | ✗ **NOT measurable from top-view tip-trail $\kappa(s)$ as written** — empirically confirmed in PR #9 (descoped to issue #230); see the PR #9 note below |
| $L_c$ via post-peak exponential decay | ✗ **NOT measurable from top-view tip-trail $\kappa(s)$ as written** — PR #9, issue #230 |
| $B = L_{gz}/L_c$ | ✗ Blocked on $L_{gz}$/$L_c$ (issue #230) |
...
**Required upstream change for the missing traits:** multi-node SLEAP skeleton tracking (e.g., 5–10 nodes equispaced along the root midline, or feature-point tracking of distinguishable surface marks). This would give $(x_i(t), y_i(t))$ for nodes $i = 1, \ldots, N$, from which $\dot{\varepsilon}(s_a, t)$ is computable via image-to-image correlation à la Bastien et al. 2016 (KymoRod) or Chavarría-Krauser et al. 2008. **This is a Phase 2+ pipeline upgrade and is outside the scope of this document.** The current pipeline, working from tip-only data, can correctly emit $\dot{\delta}_0$ and integrated $\dot{\varepsilon}_0$, but cannot emit the spatial *profiles* of these quantities along the organ.

> **PR #9 empirical correction (2026-06-09; supersedes the "✓ Measurable" rows above for $L_{gz}$/$L_c$/$B$).** A pre-spec `research:investigate` on the 6 Nipponbare plate-001 proofread tracks established that the §7.4 "$L_{gz}$ = peak of the $|\kappa(s)|$ envelope, $L_c$ = post-peak exponential decay" recipe does **not** transfer to top-view tip-trail $\kappa(s)$. The reconstructed trail is a roughly **steady-amplitude oscillation over the whole ~3000 px trail** (the QPB steady traveling wave, §4.7): the growth zone is ~1 % of the trail, and the $|\kappa|$ envelope's global argmax lands **basal** (48–100 % of span) on every track and every envelope method (Hilbert / cgau2 CWT-power / smoothed-$|\kappa|$) — exactly the "the recent trail cannot be inverted" limit stated above in this section. This is a $L_{gz}$/$L_c$-specific instance of the same degeneracy that already blocks the spatial *profiles*. PR #9 therefore ships only the **resample + cgau2 scaleogram + $\lambda_{spatial}$ ridge** (which the steady wave *does* support); $L_{gz}$/$L_c$ detection — and hence $B = L_{gz}/L_c$ and the $L_{gz}$ growth-zone mask — are **descoped to follow-up research issue #230** (redefine for tip-trail geometry, relocate to side-view/multi-node data, or mark not-measurable). The original "✓ Measurable" wording is preserved in Appendix B.
...
> **PR #9 scope note (2026-06-09).** PR #9 ships the spatial-CWT *machinery* —
> `spatial_cwt.resample_curvature` (the PR #8 non-uniform→uniform $\kappa(s)$
> handoff), `compute_scaleogram` (cgau2), and `extract_ridge` ($\lambda(s_a)$
> ridge) — and emits **no trait columns**. The growth-zone-structure traits
> below marked **[descoped → #230]** (`L_gz_estimate`, `L_c_estimate`,
> `B_balance_number`, `L_gz_steady_state_residual`, `L_gz_resolvable`) are **not
> measurable from top-view tip-trail $\kappa(s)$ as written** (see the §6.6 PR #9
> empirical correction); they await follow-up research issue #230. The
> $\lambda$-based traits (`lambda_spatial_median_px`, `traveling_wave_residual`,
> `lambda_spatial_variation` — the latter renamed from `apex_basal_period_consistency`
> in PR #10) ARE deliverable in PR #10 from PR #9's $\lambda(s_a)$ ridge — subject
> to the **Handoff to PR #10** below (notes 2 and 4 were overturned in PR #10; see
> Appendix B(6)).

| Symbol | Units | Description | Source / anchor |
|---|---|---|---|
| `L_gz_estimate` | mm `[mm]` **[descoped → #230]** | Distance from apex to peak of $|\kappa(s)|$ envelope along reconstructed midline | Rivière 2022 Eq. 4 (peak of $1-\tanh^2$ at $s_a = L_{gz}$); QPB 2025 Figure 4 |
| `L_gz_steady_state_residual` | mm `[mm]` **[descoped → #230]** | Median absolute deviation of `L_gz_estimate` measured at multiple sliding-window timepoints (recommended: at $t = $ 4, 8, 12 hr into the run, with trail truncated at each). Steady-traveling-wave hypothesis (QPB 2025) predicts the apex-frame peak position is time-invariant. **Threshold:** residual $< 0.2 \times$ `L_gz_estimate` is steady; larger residual flags hypothesis violation and reduces confidence in `L_gz_estimate`. Adds ~3 spatial-CWT calls per plant. | This doc §6.6 (steady-state assumption); QPB 2025 §"Steady propagation"; falsification check |
| `L_gz_resolvable` | bool `[—]` **[descoped → #230]** | True iff `L_gz_estimate` $\geq N_{\min} \cdot v \cdot \Delta t$ with $N_{\min} = 5$. False values mean the growth zone spans too few trail frames for reliable peak detection — see §6.4. | This doc §6.4 |
| `L_c_estimate` | mm `[mm]` **[descoped → #230]** | Exponential decay length of $|\kappa(s)|$ basal of peak: fit $|\kappa(s)| = A\exp(-s'/L_c)$ for $s' = s - L_{gz}$, $s' > 0$ | Meroz 2026 Eq. 5 (steady-state shape); Eq. 6 ($L_c = \gamma/\beta$) |
| `B_balance_number` | — `[—]` **[descoped → #230]** | $L_{gz} / L_c$ | Meroz 2026 Eq. 7; Bastien et al. 2013 cross-species baseline |
| `lambda_spatial_median_px` | px `[px]` (PR #10) | Median **cgau2-calibrated** dominant spatial wavelength from CWT of $\kappa(s)$ along the **full reconstructed trail** (no $L_{gz}$ mask — descoped, #230), using `cgau2` (matches Rivière 2022). Pure-pixel (CC-3). | Rivière 2022 §"Kinematics: fine elongation measurements"; QPB 2025 §"Spatial Fourier decomposition" |
| `traveling_wave_residual` | — `[—]` | $\|\lambda_\text{spatial} - v\,T_\text{frames}\| / (v\,T_\text{frames})$ with $T_\text{frames} = T_\text{nutation}/\texttt{cadence\_s}$ and **calibrated** $\lambda_\text{spatial}$ (true px both sides; the §7.4 "bias cancels" handoff was empirically false — Appendix B(6)). Plate-001: ~0.09–0.21 (QPB holds; provisional). | QPB 2025 §4.7 (this doc); falsifiable test of steady-traveling-wave hypothesis |
...
| `lambda_spatial_variation` (PR #10; was `apex_basal_period_consistency`) | — `[—]` | $\text{MAD}(\lambda_\text{cal})/\text{median}(\lambda_\text{cal})$ over COI-valid positions — an **orientation-robust** along-trail spread (NOT an apex-vs-basal difference: those extremes are edge-artifact-dominated and the $s_a$ orientation is ambiguous, §6.2; Appendix B(6)). ≈0 on a uniform-$\lambda$ trail (no quantization floor); real-data scatter is noise-dependent. | Bastien & Meroz 2016 §3.7; QPB 2025 §"Steady propagation" |

**Handoff to PR #10 — computing `traveling_wave_residual` from PR #9's $\lambda(s_a)$ ridge (recorded so the central falsifiable trait is not silently corrupted; PR #9 review):**

1. **Unit reconciliation (dimensional landmine).** Compare in ONE unit (px). The *emitted* operand units (NOT the `[mm]`/`hr` annotations in these tables, which are post-conversion / stale): `spatial_cwt` `wavelengths_px` = **px**; Tier-0 `v_total_median_px_per_frame` = **px/frame**; Tier-1 `T_nutation_median` = **seconds** (the column has no suffix; the "hr" in some trait tables is stale — cf. Appendix B(3), where `T_psig` was corrected to seconds for Tier-1 consistency; do **not** multiply by 3600). So `v · T` naively is px·s/frame, NOT px. The correct comparison is $\lambda_\text{spatial}[\text{px}]$ vs $v[\text{px/frame}] \cdot (T_\text{nutation}[\text{s}] / \texttt{cadence\_s}[\text{s/frame}])$ — i.e. $T_\text{frames} = T_\text{nutation}/\texttt{cadence\_s}$, matching §6.4's frame-domain derivation ($5.83 \times 3333/300 \approx 65$ px). §7.4's `[mm]` $\lambda$ is post-`convert_to_mm` only — convert *both* sides or neither.
2. **cgau2 wavelength calibration.** `spatial_cwt.SpatialScaleogramResult.wavelengths_px` is the honest `pywt.scale2frequency("cgau2", …)` convention value — it over-reports the true px wavelength by a $\lambda$- *and* $n$-dependent band, measured + emitted as a machine-readable map at `tests/data/circumnutation_spatial_cwt_calibration.json` (by `scripts/circumnutation/capture_spatial_coi_factor.py`). Do NOT bake a correction into `wavelengths_px`. **⚠ PR #10 correction (Appendix B(6)):** the original claim here — "for `traveling_wave_residual` the bias largely cancels if both sides are compared in the same scale domain" — is **empirically FALSE**. $v\,T_\text{frames}$ is a true-px prediction carrying NO cgau2 bias, so the naive `|λ_raw − v·T|/(v·T)` is mixed-domain and wrong (it only *looks* small by accident). The calibration map MUST be applied to $\lambda$ for **all** $\lambda$-consumers (including `traveling_wave_residual`), in true px on both sides. PR #10 uses an **n-averaged** `ratio(λ)` curve (the per-$n$ ratio scatters ~7 % non-monotonically; averaging avoids false precision and books a documented ~±5 % systematic) and extends the calibration table append-only to $\lambda_\text{true} \ge 140$ px.
3. **COI reliability gate.** `extract_ridge` ships the raw, un-COI-masked ridge (mirrors `temporal_cwt`). PR #10 MUST apply a spatial `COI_FRACTION_MAX`-style gate (NaN $\lambda_\text{spatial}$ when too few `~in_coi` positions survive). Any $\lambda$ in PR #9's own artifacts is **ungated / non-authoritative**.
4. **`apex_basal_period_consistency` (PR #10: renamed `lambda_spatial_variation`).** The original instruction here — compare $\lambda$ at the apex ($s_a \to 0$) vs the base ($s_a \to \max$), applying the calibration per-$\lambda$ and pinning apex = $s_a \to 0$ — is **overturned in PR #10 (Appendix B(6))**: a pre-spec investigation on all 6 plate-001 tracks found the apex/basal *extremes* are edge/scale-rail-artifact-dominated (e.g. an apex $\lambda \approx 432$ px, >3× the track median) and the $s_a=0$ orientation is ambiguous (the resample anchors $s_a=0$ at the oldest tissue, possibly opposite the growing apex; §6.2). PR #10 therefore measures the §7.4 *uniformity* intent as an **orientation-robust** spread $\text{MAD}(\lambda_\text{cal})/\text{median}(\lambda_\text{cal})$ over COI-valid positions (the statistic is reversal-invariant), still on the calibrated $\lambda$. Empirically it reads ≈0 on a noise-free uniform-$\lambda$ trail (no spurious argmax-quantization floor) and grows with curvature-localization noise.
...
**(4) §6.2 curvature sign clarified during PR #8 (Tier 3a) implementation.** Original: *"Sign convention: positive = left turn, negative = right turn."* This is correct in **y-up math axes** (which the formula $\kappa = (\dot{x}\ddot{y} - \dot{y}\ddot{x})/|\mathbf{v}|^3$ encodes), but the pipeline runs in the **y-down image frame**, where $+\kappa$ is a clockwise / visual-right turn *as displayed*. The `_geometry.compute_path_curvature` implementation (PR #8) anchors on the formula sign via an absolute hand-built test (`compute_path_curvature([1],[0],[0],[1]) == +1.0`), NOT the frame-ambiguous word "left turn" — exactly as `compute_signed_area` anchors `handedness`. The cross-helper identity $d\psi_g/dt = -\kappa\,|\mathbf{v}|$ (the $\psi_g$ family uses the swapped `atan2(dx, dy)`) gives $\mathrm{sign}(\kappa) = -\,\texttt{handedness}$ for a single-signed-$\kappa$ loop; this is recorded so a PR #9/#10 chirality trait composing $\kappa$ with `handedness` does not silently invert. Full rationale + the 4-round review reconciliation: `docs/superpowers/specs/2026-06-08-add-circumnutation-tier3a-midline-design.md` §13 and the OpenSpec change `add-circumnutation-tier3a-midline`.

**(5) $L_{gz}$/$L_c$ growth-zone-structure traits descoped during PR #9 (Tier 3b) implementation.** The §6.6 trait-status table originally listed *"$L_{gz}$ via peak-$|\kappa(s)|$ location — ✓ Measurable (Tier 3)"*, *"$L_c$ via post-peak exponential decay — ✓ Measurable (Tier 3)"*, and *"$B = L_{gz}/L_c$ — ✓ Measurable (Tier 4)"*, and §7.4 defined `L_gz_estimate`/`L_c_estimate`/`B_balance_number`/`L_gz_steady_state_residual`/`L_gz_resolvable` as emitted traits. A pre-spec `research:investigate` (2026-06-09) on the 6 Nipponbare plate-001 proofread tracks found this recipe does **not** transfer to top-view tip-trail $\kappa(s)$: the reconstructed trail is a steady-amplitude oscillation over the whole ~3000 px (the QPB steady traveling wave, §4.7), the growth zone is ~1 % of it, and the $|\kappa|$ envelope's global argmax lands basal on every track and every envelope method (Hilbert / cgau2 CWT-power / smoothed-$|\kappa|$) — the §6.6 "recent trail cannot be inverted" limit, applied to $L_{gz}$/$L_c$ specifically. The §6.6 rows and §7.4 entries were flipped to **✗ / [descoped → #230]** and PR #9 ships only the resample + cgau2 scaleogram + $\lambda_{spatial}$ ridge (which the steady wave supports). The original "✓ Measurable" wording is preserved here for provenance / reproducibility of any analysis citing the prior definitions. Redefinition (for tip-trail geometry), relocation (to side-view / multi-node data), or formal not-measurable marking is tracked in GitHub issue #230. Full rationale + the 4-round review reconciliation: `docs/superpowers/specs/2026-06-09-add-circumnutation-tier3b-spatial-cwt-design.md` and the OpenSpec change `add-circumnutation-tier3b-spatial-cwt`.

**(6) Two §7.4 "Handoff to PR #10" claims overturned + a trait renamed during PR #10 (Tier 3c) implementation.** A pre-spec `research:investigate` (2026-06-10) on the 6 Nipponbare plate-001 proofread tracks (`docs/circumnutation/investigations/2026-06-10-tier3c-traveling-wave/report.md`) overturned two handoff-note claims and one trait definition; the originals are preserved here for provenance:

- **Handoff note 2 — "the bias largely cancels in the ratio" was FALSE.** Original: *"For `traveling_wave_residual` (a ratio with λ on one side) the bias largely cancels if both sides are compared in the SAME (scale) domain; for absolute-λ consumers (`lambda_spatial_median`) the calibration map MUST be applied."* Empirically, $v\,T_\text{frames}$ is a true-px prediction carrying no cgau2 bias, so the raw mixed-domain residual is wrong — it reads ~0.05 only because the +13 % over-report accidentally nudges $\lambda_\text{raw}$ toward $v\,T$. The honest, calibrated residual is ~0.09–0.21 on plate-001. Correction: calibrate $\lambda$ to true px for **all** consumers (including the residual). PR #10 additionally uses an **n-averaged** `ratio(λ)` curve (per-$n$ scatter ~7 %, non-monotone) with a documented ~±5 % systematic, shipped as an in-package literal, and extends the calibration table append-only to $\lambda_\text{true} \ge 140$ px.

- **Handoff note 4 + the `apex_basal_period_consistency` trait — apex-vs-basal was unmeasurable; renamed `lambda_spatial_variation`.** Original §7.4 definition: *"`apex_basal_period_consistency` — Whether spatial CWT shows uniform λ(s) along trail; large variation = H1 violation,"* with handoff note 4 instructing an apex ($s_a\to0$) vs base ($s_a\to\max$) comparison. The investigation found the $s_a$ extremes are edge/scale-rail-artifact-dominated (an apex $\lambda\approx432$ px, >3× the track median) and the $s_a=0$ orientation is ambiguous (resample anchors $s_a=0$ at the oldest tissue; §6.2). Correction: the §7.4 *uniformity* intent is measured as an **orientation-robust** spread $\text{MAD}(\lambda_\text{cal})/\text{median}(\lambda_\text{cal})$ over COI-valid positions (reversal-invariant statistic), renamed `lambda_spatial_variation`. It reads ≈0 on a noise-free uniform-$\lambda$ synthetic (there is **no** spurious argmax-quantization floor — a round-3 hypothesis that was itself empirically refuted) and grows with curvature-localization noise.

- **Units / scope (already consistent with the §7.4 row edits):** `lambda_spatial_median` → `lambda_spatial_median_px` (px, pure-pixel CC-3, not mm); no $L_{gz}$ mask (the trail-wide median replaces "basal of $L_{gz}$"). The plate-001 QPB result (~9–21 %) is provisional pending any further calibration refinement.

Full rationale + the 4-round review reconciliation: `docs/superpowers/specs/2026-06-10-add-circumnutation-tier3c-traits-design.md` and the OpenSpec change `add-circumnutation-tier3c-traits`.
