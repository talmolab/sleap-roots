...
> $\lambda$-based traits (`lambda_spatial_median`, `traveling_wave_residual`,
> `apex_basal_period_consistency`) ARE deliverable in PR #10 from PR #9's
> $\lambda(s_a)$ ridge — subject to the **Handoff to PR #10** below.
...
2. **cgau2 wavelength calibration.** `spatial_cwt.SpatialScaleogramResult.wavelengths_px` is the honest `pywt.scale2frequency("cgau2", …)` convention value — it over-reports the true px wavelength by a $\lambda$- *and* $n$-dependent band (≈+4–16 %, NOT a single factor), measured + emitted as a machine-readable map at `tests/data/circumnutation_spatial_cwt_calibration.json` (by `scripts/circumnutation/capture_spatial_coi_factor.py`). For `traveling_wave_residual` (a *ratio* with $\lambda$ on one side) the bias largely cancels if both sides are compared in the SAME (scale) domain; for **absolute-$\lambda$** consumers (`lambda_spatial_median`) the calibration map MUST be applied. Do NOT bake a correction into `wavelengths_px`.
...
4. **`apex_basal_period_consistency` — the $\lambda$-offset does NOT cancel here.** It compares $\lambda$ at the apex ($s_a \to 0$, short $\lambda$) vs the base ($s_a \to \max$, long $\lambda$) on ONE scaleogram; because the cgau2 over-report is $\lambda$-dependent, apex and basal positions get *different* fractional offsets → a genuinely uniform-$\lambda$ trail shows spurious variation (false H1 violation). PR #10 MUST apply the calibration map per-$\lambda$ before comparing, and pin the direction **apex = $s_a \to 0$, base = $s_a \to \max$** (do not infer apex from raw `arc_length`; recall the §6.2 `sign(κ) = −handedness` polarity warning).
