...
2. Derive an integer `track_id` by stripping a **prefix-anchored** `"track_"` (e.g. `removeprefix("track_")` / a `^track_` match — NOT a global `str.replace`, which would corrupt an interior occurrence). The remainder MUST be an **unsigned run of ASCII digits** (`re.fullmatch(r"[0-9]+", ...)`, not `\d+`) — this rejects both non-numeric remainders (`"track_2a"`, `"track_track_1"`) AND names that Python's `int()` would silently mis-coerce (signed `"track_-1"`, whitespace-padded `"track_ 1"`, underscore-separated `"track_1_2"` → `12`, and non-ASCII Unicode decimal digits). Any name that does not match SHALL raise `ValueError` naming the offending track name(s) rather than be silently coerced.
