...
- [x] 1.1 RED: extend `tests/test_circumnutation_foundation.py` — add `"traveling_wave"` (with a
...
- [x] 1.2 GREEN: create `sleap_roots/circumnutation/traveling_wave.py` with module docstring
...
- [x] 2.1 RED: schema test — `compute` returns 8 `ROW_IDENTITY_COLUMNS` + 6 trait columns in the
...
- [x] 2.2 GREEN: implement the emission tail verbatim from siblings — `groupby(list(_IDENTITY_5_TUPLE),
...
- [x] 2.3 RED: units-vocabulary test (mirror `test_circumnutation_psi_g.py` §2) — assert
...
- [x] 2.4 RED: logging test — `caplog.set_level(logging.DEBUG, logger="sleap_roots.circumnutation.traveling_wave")`;
...
- [x] 3.1 RED: per-track tests — (a) a healthy single track yields finite λ traits; (b) a degenerate
  ...
  stays finite.
- [x] 3.2 GREEN: implement `_compute_one_track`: sort by `frame` + drop NON-finite tips
...
- [x] 4.1 RED: COI gate boundary test — construct (synthetic OR monkeypatched) `SpatialRidgeResult`s
...
- [x] 4.2 RED: in-package calibration-literal sync test — assert the module-level
...
- [x] 4.3 GREEN: implement `~in_coi` interior selection, the COI fraction gate
...
- [x] 5.1 RED: **multi-plate test with float64 `track_id`** (≥2 plates, overlapping track_ids) —
...
- [x] 5.2 GREEN: recompute `kinematics.compute(df, constants=resolved)` and
...
- [x] 6.1 RED: regression test asserting the existing 18 `(n, λ_true)` rows + the entire `provenance`
...
- [x] 6.2 GREEN: add an append-only / merge mode to `capture_spatial_coi_factor.py` that **reads the
...
- [x] 6.3 Update the stale `# … ratio [1.044, 1.156]` comment in `tests/test_circumnutation_spatial_cwt.py`
...
- [x] 7.1 RED: determinism — (a) two runs of the **full composed chain (incl. the np.interp
...
- [x] 7.2 RED: real-data plate-001 test (all 6 tracks), **gated with
...
- [x] 7.3 RED: synthetic known-λ recovery — a **small-amplitude, low-noise** trajectory
...
- [x] 7.4 GREEN: reconcile any numeric drift; lock the measured atol + canary values.

## 8. Spec deltas + docs deviations (CR-12, CR-13, CR2-6)
- [x] 8.1 Author spec deltas: MODIFY "Package layout" (add `traveling_wave`, 8→9 impl, PR #10
...
- [x] 8.2 theory.md edits: §7.4 rows for the 3 λ-traits (lines 504/505/507 — rename
...
- [x] 8.3 roadmap.md line 146: rename both `apex_basal_period_consistency` → `lambda_spatial_variation`;
...
- [x] 9.1 `uv run pytest tests/ -q` green; `black --check`; `pydocstyle --convention=google
...
- [x] 9.2 Coverage reasoning (CI-enforced ≥84% project, aim ≥90% on the new module); confirm via
