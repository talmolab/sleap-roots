...
- **AND** the ridge `wavelengths_px[interior]` array (a PUBLIC `SpatialRidgeResult` field, 1:1 with the per-position `argmax` scale-index selection) is captured and asserted EXACTLY equal cross-OS — because an argmax tie-flip is a DISCRETE full-scale-step jump, not an `atol`-bounded perturbation, so a float tolerance alone would not characterize cross-OS reproducibility; this exact-equality of the spatial-λ ridge is the load-bearing determinism contract (the integer scale index itself is not exposed on the Result, so the equivalent `wavelengths_px` equality is asserted instead)
