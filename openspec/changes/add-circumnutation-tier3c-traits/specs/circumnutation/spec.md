...
- **Documented calibration limitation.** The cgau2 calibration carries an irreducible ~±5 % systematic (the residual `n`-scatter after averaging). `traveling_wave_residual` differences between tracks that are within ~5 %, and `lambda_spatial_variation` values within its measured argmax-quantization noise floor (see below), SHALL be documented (spec + theory) as "within calibration/quantization uncertainty" and not over-interpreted. The headline plate-001 result is provisional pending the post-extension re-measurement (see the real-data scenario).
- **Trait definitions.** `lambda_spatial_median_px = median(λ_cal[interior])`; `lambda_spatial_mad_px = median(|λ_cal[interior] − median|)`; `lambda_spatial_variation = lambda_spatial_mad_px / lambda_spatial_median_px` (an **orientation-robust** spread — the median/MAD statistic is invariant under arc-length reversal, though the COI-interior selection still depends on the resample `s_a=0` anchor; 0 = uniform); `lambda_expected_px = v_total_median_px_per_frame · (T_nutation_median / cadence_s)`; `traveling_wave_residual = |lambda_spatial_median_px − lambda_expected_px| / lambda_expected_px`. `lambda_spatial_variation` has a nonzero **noise floor** set by the per-position argmax quantizing onto the log-spaced CWT scale grid; values within that measured floor are "consistent with uniform" (it is a spread diagnostic, not a calibrated H1 test below the floor). `traveling_wave_residual` is interpretable only when `lambda_expected_px` is at least ~one resolvable wavelength; a tiny-but-positive `v·T_frames` yields a large-but-finite residual that is an undefined-regime artifact, not a QPB violation (see the gating below and the documented limitation).
...
- **AND** the integer ridge scale-index array (the per-position `argmax` selections feeding the calibrated λ) is captured and asserted **equal** cross-OS — because an argmax tie-flip is a DISCRETE full-scale-step jump, not an `atol`-bounded perturbation, so a float tolerance alone would not characterize cross-OS reproducibility; index-equality is the load-bearing determinism contract
