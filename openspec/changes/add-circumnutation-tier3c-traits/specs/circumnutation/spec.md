...
#### Scenario: in-package calibration literal matches the authoritative JSON n=400 slice
- **GIVEN** the module-level `_CGAU2_LAMBDA_CALIBRATION_N400` literal and the authoritative `tests/data/circumnutation_spatial_cwt_calibration.json`
- **WHEN** the JSON's `n == 400` calibration entries are extracted and sorted by `λ_reported`
- **THEN** the literal's `(λ_reported, ratio)` pairs equal the JSON's n=400 entries (to `atol=0` on the committed values)
- **AND** the literal's `λ_reported` axis is strictly increasing (well-posed `np.interp`) and covers `λ_true` up to ≥ 140 px (no clamped extrapolation for the observed real λ ≈ 142 px)
