...
When it writes any plot, `save_plots` SHALL also write a `plots/plots_metadata.json` provenance sidecar recording: the constants version (`_CONSTANTS_VERSION`), the resolved plot display constants (κ percentile, colormap names, DPI, figure sizes), the per-plant source identity tuples for the plants plotted, the list of written PNG filenames, and a relative back-reference to the run's `run_metadata.json`. The sidecar SHALL be strict-JSON (finite floats; numpy/`Path` values coerced to native) — mirroring the sleap-roots-analyze serialization convention (#241) — so the PNGs are traceable to the run and constants that produced them. When `enabled=False` (no plots written) no sidecar SHALL be written.
...
- **AND** it records the constants version, the resolved plot display constants (κ percentile, colormaps, DPI, figure sizes), the per-plant source identity tuples, the written PNG filenames, and a back-reference to the run's `run_metadata.json`
