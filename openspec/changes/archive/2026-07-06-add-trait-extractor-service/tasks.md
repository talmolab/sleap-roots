...
- [x] 1.1 **Pre-flight (hard gate), explicit ordering:** FIRST edit BOTH dependency locations —
...
- [x] 1.2 Set `[tool.pytest.ini_options] pythonpath = ["."]`. Wheel/sdist exclusion of
...
- [x] 1.3 Create the flat `trait_extractor/` package (`__init__.py`) and `tests/trait_extractor/`.
  **Test first:** `import trait_extractor` passes.
- [x] 1.4 **Test first (CI-enforced boundary guard, AST-based):** a `pytest` test parses every
...
- [x] 2.1 Author a **nested** per-scan layout under `tests/data/rice_3do_pipeline_output/` mirroring
...
- [x] 2.2 Golden source confirmed reusable: the pipeline-output `.slp` are **byte-identical**
...
- [x] 3.1 **Test first:** consumer `PredictionManifest`/`PredictionArtifact` (importing `ModelRef`
...
- [x] 3.2 **Test first:** `schema_version` is pinned `Literal["1"]` — a manifest with
  `schema_version:"2"` raises; an unknown `root_type` raises `ValidationError`. Then confirm.
- [x] 3.3 **Test first:** cross-check — if `sleap_roots_predict.output_contract` is importable, the
  consumer model accepts its real `model_dump_json`; else `pytest.skip`.
- [x] 3.4 **Test first (negative paths; guards raise — no broad `except` in `manifest.py`):**
  ...
  Then implement.
- [x] 3.5 **Test first (`ScanMetadata` model + canonical params — the reproducibility core):**
...
## 4. Series loading
- [x] 4.1 **Test first:** `Series.load(series_name=scan_key, primary_path=…, crown_path=…)` returns
...
- [x] 4.2 **Test first (empty-artifacts guard, extractor-owned, BEFORE `Series.load`):** because
...
- [x] 5.1 **Test first:** a public, test-constructible `PipelineCard` type + `pipeline_selection.yaml`
...
- [x] 5.2 **Test first:** pass an in-memory `cards` list (DI — no YAML shadowing): override wins;
...
- [x] 6.1 **Test first (grain guard FIRST):** selecting a multi-plant / plate pipeline
...
- [x] 6.2 **Test first (subset compatibility):** `assert_pipeline_compatible(series, pipeline_cls)`
...
- [x] 6.3 **Test first (guard test pins the map independently):** for every mapped pipeline,
...
- [x] 7.1 **Test first:** `pipeline_cls().compute_batch_traits([series]).iloc[0].to_dict()` →
  `list[TraitValue]` with `grain="scan"`, correct `scan_key`, `plant_name` excluded. Then implement.
- [x] 7.2 **Test first:** a `NaN`/`inf` trait → `TraitValue.value is None`. Then confirm.

## 8. Provenance + envelope emission (+ golden regression)
- [x] 8.1 **Test first:** `Provenance` has `predict_models == [a.model for a in artifacts]`,
...
- [x] 8.2 **Test first:** `idempotency_key` non-empty, equals `compute_idempotency_key(...)`, stable
  across repeated assembly; `traits_code_sha`/`traits_container_digest` fail-soft (arg → env → `""`).
  Then confirm.
- [x] 8.3 **Test first:** end-to-end `extract_scan` (creates `output_dir` `parents=True,
...
- [x] 8.4 **Test first (golden regression):** for `scan0K9E8BI`, compare an explicit shared
...
## 9. Batch driver + CLI
- [x] 9.1 **Test first:** `extract_batch(input_dir, output_dir)` **recursively** discovers
...
- [x] 9.2 **Test first (failure isolation — broad `except` ONLY here):** a batch with one valid
...
- [x] 9.3 **Test first:** `python -m trait_extractor <input_dir> <output_dir>` (via
  `subprocess`/`runpy`) writes the envelopes. Then implement `__main__.py`.

## 10. CI wiring & verification
- [x] 10.1 Extend `.github/workflows/ci.yml`: add `trait_extractor/**` to path filters in BOTH the
...
- [x] 10.2 Decide coverage scope: `trait_extractor` is not counted under `--cov=sleap_roots` —
  either add `--cov=trait_extractor` or record the omission as intentional. **Verify:** reflected
  in `ci.yml`.
- [x] 10.3 Run the full local gate mirroring CI: `uv run black --check sleap_roots tests
...
- [x] 10.4 Reconcile implementation against `proposal.md` / `spec.md` / `design.md` / this file;
  record any deviation with a `### Why N instead of M?` note before committing.

## 11. Documentation
- [x] 11.1 Add a `docs/changelog.md` `[Unreleased]` entry (repo convention): under `### Added`, the
...
- [x] 11.2 Add a durable **`docs/dev/trait-extractor-service.md`** page (in `mkdocs.yml` under
...
- [x] 11.3 Record that `trait_extractor` is intentionally excluded from the generated API reference
  (`docs/gen_ref_pages.py` walks only `sleap_roots/`).
- [x] 11.4 Update the brainstorming design doc's reconciliation appendix if any decision changed
