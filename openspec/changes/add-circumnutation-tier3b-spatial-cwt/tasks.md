...
- [ ] 2.7 RED: non-finite + degenerate behavior — non-finite `(curvature, arc_length)` pairs are DROPPED (not rejected), no `RuntimeWarning`; degenerate gate (too-few survivors / non-positive span / all-equal arc_length / **fully-masked `n_unmasked == 0`**) → graceful all-NaN `ResampleResult` (`is_degenerate=True`). The gate runs from survivor-count+span ALONE, BEFORE the apex-origin `max`/`min` AND `np.median(positive Δs_a)`, so an empty survivor set never hits `np.max([])`/`np.min([])` and an all-equal arc_length never hits `np.median([])`.
- [ ] 2.8 GREEN: implement drop-non-finite + the gate-before-max/min/median ordering (`np.errstate` + `~isfinite` sweep).
...
- [ ] 8.6 Add a "Handoff to PR #10" note in `docs/circumnutation/theory.md` §7.4 (next to `traveling_wave_residual`) covering the three round-3 downstream risks: (1) the **unit reconciliation** `λ_spatial[px]` vs `v[px/frame]·(T[s]/cadence_s)` — §6.4 uses T in frames but the pipeline emits `periods_s`; the §7.4 mm annotation is post-`convert_to_mm` only; (2) the cgau2 `scale2frequency` **wavelength calibration** is a λ/n-dependent band (PR #9 ships the convention value; PR #10 must reconcile identically on both sides of the residual); (3) PR #10 MUST apply a spatial `COI_FRACTION_MAX` reliability gate on λ_spatial. (Design rationale: change `design.md` "Handoff to PR #10".)
