...
- [ ] 2.7 RED: non-finite + TWO-STAGE degenerate gate, no `RuntimeWarning` (assert under `warnings.simplefilter("error")`, PR #8 parity): non-finite `(curvature, arc_length)` pairs are DROPPED (not rejected); **Stage 1** (too-few survivors / non-positive span / all-equal arc_length / fully-masked `n_unmasked == 0`) short-circuits BEFORE apex-origin `max`/`min` AND `np.median(positive Δs_a)`; **Stage 2** (skewed gaps → `floor(arc_span/ds)+1 < MIN_SAMPLES_REQUIRED`, e.g. gaps `[1,1,1,100,100,100,100,100]` with 9 survivors → grid length 6) → `is_degenerate=True`. → graceful all-NaN `ResampleResult`.
- [ ] 2.8 GREEN: implement drop-non-finite + the two-stage gate (Stage 1 from count+span before max/min/median; Stage 2 output-grid-length ≥ MIN after `ds`), `np.errstate` + `~isfinite` sweep. Guarantees `len(kappa_uniform) ≥ MIN_SAMPLES_REQUIRED` for any non-degenerate result (the `compute_scaleogram` invariant).
- [ ] 2.7b RED: a `velocity_sub_noise_mask` given as an int 0/1 array coerces identically to a bool mask (same `n_unmasked`/`kappa_uniform`); a wrong-length mask raises `ValueError` naming the field.
- [ ] 2.8b GREEN: implement mask coercion + length check.
