...
- **WHEN** `extract_ridge(scaleogram_result)` is invoked
- **THEN** the median of `result.wavelengths_px` over the COI-interior positions (`~result.in_coi`) equals `lambda_true` to within a documented numeric tolerance (one adjacent-scale step of the 64-scale log grid, ≈ ±5% — stated as a literal in the test so a future `CWT_SCALE_COUNT_DEFAULT` change forces a conscious tolerance update)

#### Scenario: extract_ridge tracks the fundamental, not the second harmonic (cgau2 high-frequency bias)
- **GIVEN** a uniform-grid `kappa` that is a fundamental sinusoid of wavelength `lambda_true` plus a 30–40% second-harmonic component (wavelength `lambda_true/2`), passed through `compute_scaleogram(kappa, ds)`
- **WHEN** `extract_ridge(scaleogram_result)` is invoked
- **THEN** the COI-interior median of `result.wavelengths_px` tracks the fundamental `lambda_true`, NOT `lambda_true/2`
- **AND** this guards against the ridge locking onto a harmonic — a real risk because `cgau2` (a 2nd-derivative wavelet) amplifies high-frequency content
...
- **AND** any systematic bias from linear interpolation across the dropped-sample gaps is recorded for auditability — this is the robustness check on the headline data property (real κ(s) is ~50% sparse) for the one quantity PR #9 ships
