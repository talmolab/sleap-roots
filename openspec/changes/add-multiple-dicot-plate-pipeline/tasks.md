...
- [ ] 3a.1 Add `test_multiple_dicot_plate_pipeline_define_traits` — instantiate the pipeline and assert:
...
- [ ] 3a.1b Add `test_multiple_dicot_plate_pipeline_top_level_import` — verify `from sleap_roots import MultipleDicotPlatePipeline` succeeds AND the imported symbol `is sleap_roots.trait_pipelines.MultipleDicotPlatePipeline`.
...
- [ ] 3a.3 Add `test_multiple_dicot_plate_pipeline_get_initial_frame_traits_sleap_idxs` — synthetic primaries of shape `(4, 6, 2)` with instance 1 all-NaN; assert:
...
- [ ] 3b.4 Add `test_multiple_dicot_plate_pipeline_expected_count_mismatch`. Build a synthetic `.slp` with 3 detected primaries. Load the Series with a CSV specifying `expected_count=2`. Wrap the call in `with pytest.warns(UserWarning, match="expected"):` to verify the mismatch warning. Assertions:
  ...
  - exactly one `UserWarning` captured per series, message naming both 3 and 2
- [ ] 3b.4b Add `test_multiple_dicot_plate_pipeline_expected_count_match`. Same setup but `expected_count=3`. Assert no `UserWarning` emitted (use `warnings.catch_warnings(record=True)` + assert list is empty or no `UserWarning`), every `plants[i]["count_validated"] is True`, `plants[i]["count_mismatch"] is False`.
- [ ] 3b.5 Add `test_multiple_dicot_plate_pipeline_empty_frame`. Build a synthetic `.slp` where every primary instance in frame 0 is all-NaN. Note: the synthetic `.slp` builder helper MUST use a skeleton with **≥3 nodes** (to avoid crashing `get_node_ind` — see issue #150) AND must not rely on `Series.get_primary_points`'s NaN-placeholder `(1, 2, 2)` shape (the test constructs explicit all-NaN instances with the matching node count). Call `compute_plate_traits(series)`. Assert `plants == []`, no exception.
- [ ] 3b.5b Add `test_multiple_dicot_plate_pipeline_zero_laterals`. Build a synthetic `.slp` with ONE primary at known coordinates and ZERO lateral instances. Call `compute_plate_traits(series)`. Assertions (Req 3 scenario "Zero-laterals plant yields lateral_count == 0"):
...
- [ ] 3b.6 Add `test_multiple_dicot_plate_pipeline_timelapse_shape`. Build a synthetic `.slp` with 2 frames, each containing the same 3 primaries. Call `compute_plate_traits(series)`. Assertions (Req 3 scenario "Multi-frame series produces N_plants × N_frames rows grouped by frame"):
  - `len(plants) == 6`
...
- [ ] 3b.7 Add `test_multiple_dicot_plate_pipeline_csv_output`. Build a synthetic `.slp` with 2 plants. Call `compute_plate_traits(series, write_csv=True, output_dir=tmp_path)`. Read via `df = pd.read_csv(tmp_path / f"{series.series_name}.plate_traits.csv")`. Assertions:
...
- [ ] 3b.7b Add `test_multiple_dicot_plate_pipeline_csv_missing_expected_count`. Series loaded without CSV (expected_count=NaN), 1 detected primary. After `compute_plate_traits(series, write_csv=True, output_dir=tmp_path)` + `pd.read_csv`, assert `pd.isna(df.loc[0, "expected_count"])` AND `df.loc[0, "detected_count"] == 3` (wait — 1 primary, not 3 — test uses 1 plant, so `detected_count == 1`).
- [ ] 3b.8 Add `test_multiple_dicot_plate_pipeline_json_output`. Call `compute_plate_traits(series, write_json=True, output_dir=tmp_path)`. Read back via `json.load`. Assertions (Req 4 scenarios):
  - top-level keys `{schema_version, units, series, group, qc_fail, expected_count, plants}`
  - `result["schema_version"] == 1` AND `result["units"] == "pixels"`
  ...
  - each plant entry has `count_validated` and `count_mismatch` as JSON-native booleans (not int 0/1)
  - `traits` dict keys include DicotPipeline trait names unchanged
- [ ] 3b.8b Add `test_multiple_dicot_plate_pipeline_json_rfc8259_valid`. Setup that produces at least one NaN value (Series without CSV). After `compute_plate_traits(series, write_json=True, output_dir=tmp_path)`:
  - `"NaN" not in tmp_path.joinpath(f"{series.series_name}.plate_traits.json").read_text()` (string-grep assertion)
  - `json.loads(text, parse_constant=lambda s: (_ for _ in ()).throw(ValueError(s)))` MUST NOT raise (strict RFC-8259 parse check — any bare NaN would trigger `parse_constant`)
  - after re-loading, `loaded["expected_count"] is None` (NaN was emitted as JSON null)
- [ ] 3b.9 Add `test_compute_batch_plate_traits`. Build two synthetic `.slp` Series (2 plants × 1 frame, 3 plants × 1 frame). Call `compute_batch_plate_traits([seriesA, seriesB])`. Assert returned DataFrame has 5 rows, `series` column matches expectations for rows 0-1 vs 2-4. Call again with `write_json=True, output_dir=tmp_path, json_name="batch.json"` and assert `json.loads((tmp_path / "batch.json").read_text())` is a list of 2 per-series dicts, each with `{schema_version, units, series, group, qc_fail, expected_count, plants}` keys. Also assert the batch JSON passes the same RFC-8259 strict-parse check as 3b.8b.
- [ ] 3b.10 Run `uv run pytest tests/test_trait_pipelines.py -k multiple_dicot_plate_pipeline -v` — confirm all **15** tests (3a.1, 3a.1b, 3a.2, 3a.3, 3b.1, 3b.2, 3b.3, 3b.4, 3b.4b, 3b.5, 3b.5b, 3b.5c, 3b.6, 3b.7, 3b.7b, 3b.8, 3b.8b, 3b.9) FAIL with `AttributeError` / `ImportError` (class does not yet exist). Count check: 3a has 4 tests (3a.1, 3a.1b, 3a.2, 3a.3); 3b has 14 tests (3b.1 through 3b.9 plus subs 4b, 5b, 5c, 7b, 8b). Total = 18. Paste the failing-test summary into the PR body under a `## TDD evidence` heading so reviewers can verify TDD discipline (Subagent 4 Finding I6).
...
- [ ] 4.1 Add `MultipleDicotPlatePipeline` class with `@attrs.define` decorator. `define_traits()` returns the 5-element TraitDef DAG described in section 3a.1. Google-style docstring at class level MUST note:
  - (a) plates skip count-filter (D2)
  - (b) `plant_id` is a left-to-right ordering paired with original SLEAP indices (D3); `plant_id` is NOT stable across SLEAP model re-prediction — use `primary_sleap_idx` for cross-run back-mapping
...
- [ ] 4.3 Implement `compute_plate_traits(series, write_csv=False, write_json=False, output_dir=".", csv_suffix=".plate_traits.csv", json_suffix=".plate_traits.json")`. Frame loop follows design doc § "Compute flow (per series)".
  ...
  - **SLEAP index back-mapping (Req 3 duplicate-coord scenario)**: do NOT rely on `np.array_equal` first-match. Instead, track SLEAP indices alongside the distance-based association: in `compute_plate_traits`, rebuild the primary-lookup loop using the same `LineString.distance` logic as `associate_lateral_to_primary` ([points.py:511-593](../../../sleap_roots/points.py#L511-L593)) — but this time maintain a `lateral_sleap_idx_by_primary: Dict[int, List[int]]` map. For each valid lateral at post-filter index `i` (SLEAP index `initial["lateral_sleap_idxs"][i]`), compute the nearest primary and append the SLEAP index to that primary's list. This avoids identity-matching on coordinate arrays entirely and is robust to pathological duplicate-coordinate laterals.
  - **Count-flag derivation (Req 3 count_mismatch scenario, design D5b)**: compute `count_validated` and `count_mismatch` booleans per plant from `expected_count` / `detected_count`; emit as JSON fields (NOT CSV columns).
  - **Warning log (design D5b)**: if any plant row in a frame has `count_mismatch=True`, emit exactly one `warnings.warn(f"MultipleDicotPlatePipeline: {series.series_name} detected {detected_count} primaries but expected {expected_count}; no frames dropped", UserWarning)` per series (dedupe by series_name, not per-plant).
  - **Top-level JSON metadata**: include `schema_version: 1` and `units: "pixels"` in the top-level dict (design D5).
- [ ] 4.4 Implement a plate-aware JSON encoder (either subclass `NumpyArrayEncoder` at [trait_pipelines.py:123](../../../sleap_roots/trait_pipelines.py#L123) or add a sibling `PlateJSONEncoder`). It MUST emit Python `float('nan')` / numpy NaN scalars as JSON `null`. Invoke via `json.dump(result, f, cls=PlateJSONEncoder, allow_nan=False, ensure_ascii=False, indent=4)` — `allow_nan=False` forces raising if any NaN slips through the encoder (defense-in-depth). Post-write verification: after calling `compute_plate_traits(write_json=True)` in an internal debug check, the implementation MAY assert `"NaN" not in open(path).read()` — but the authoritative test is 3b.8b.
- [ ] 4.5 Implement CSV flattening: build per-plant rows where each row contains the 6 metadata columns (`series`, `frame`, `plant_id`, `primary_sleap_idx`, `expected_count`, `detected_count`) + each DicotPipeline csv-trait (scalar as-is; non-scalar via `get_summary` → `{name}_{min,max,...}` suffixes from `sleap_roots/summary.py`). Use `pd.DataFrame(rows).to_csv(path, index=False)` — pandas renders `np.nan` / `None` as empty cells by default. `count_validated` and `count_mismatch` MUST NOT appear as CSV columns (JSON only).
- [ ] 4.6 Implement `compute_batch_plate_traits(all_series, write_csv=False, write_json=False, output_dir=".", csv_name="plate_batch_traits.csv", json_name="plate_batch_traits.json")`. Loop over series, concatenate per-series DataFrames, optionally write CSV/JSON (JSON uses the same plate encoder from 4.4).
- [ ] 4.7 Add `MultipleDicotPlatePipeline` to `sleap_roots/__init__.py` top-level re-exports, parity with `DicotPipeline`, `MultipleDicotPipeline`, etc. (Req 1 scenario "Class is importable from the top-level sleap_roots namespace"; Subagent 2 Finding I7).
- [ ] 4.8 Run `uv run pytest tests/test_trait_pipelines.py tests/test_points.py -k "multiple_dicot_plate_pipeline or argsort_primaries_by_base_x" -v` — all 18 new tests pass.
- [ ] 4.9 Run `uv run pytest tests/ -x` — **full** test suite passes (broadened from the previous narrow scope per Subagent 1 Finding; catches ripple effects in `test_series.py`, `test_summary.py`, etc. before section 5 files follow-up issues).
