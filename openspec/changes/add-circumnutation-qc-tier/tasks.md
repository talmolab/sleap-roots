...
- [ ] 2.B.1 Test `clean_straight_line_track`: 100-frame track with `tip_x = frame * 1.0`, `tip_y = 0.0`. Expected:
  ...
  - All 3 pairwise agreements have NaN/0 issues — when both operands are 0, the ratio is `0/0 = NaN`; assert each agreement is NaN AND that the corresponding `*_agreement_high` clauses fire (because `not (NaN < threshold)` is True) — `track_is_clean = False`, `qc_failure_reason` contains all three `*_agreement_high` clauses
  - `frac_outlier_steps == 0.0`, `worst_step_ratio ≈ 1.0`
  - `growth_axis_unreliable == False`
  - **Note**: a noiseless straight-line track is NOT `track_is_clean = True` under this algorithm because the noise estimators all return 0 and divide-by-zero produces NaN agreements. This is a real edge case — real data always has SG/d2/MSD ≳ 0.1 px from quantization noise alone. Test asserts the documented behavior; if Elizabeth prefers different semantics for the 0/0 case, surface during implementation review.
