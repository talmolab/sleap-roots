...
def test_get_root_vectors_invalid_input():
    ...
    # Input has shape greater than 2 dimensions
    with pytest.raises(ValueError):
        vec1 = np.array([[[2, 2], [4, 4], [6, 6]]])
        vec2 = np.array([[3, 3]])
        get_root_vectors(vec1, vec2)
...
def test_get_nodes_invalid_input(pts_2d, pts_nan32_5node):
    ...
    # Single instance, 5 node, float index
    ...
    # Multiple instance, 5 node, array indexes contains float values
    with pytest.raises(TypeError):
        get_nodes(pts_nan32_5node, node_index=np.array([1, 1.5]))
